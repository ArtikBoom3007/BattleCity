	...
	void PhysicsEngine::update(const double delta) {
		for (auto& currentObject : m_dynamicObjects) {
			if (currentObject->getCurrentVelocity() > 0) {
				...
				for (const auto& currentObjectsToCheck : objectToCheck) {
					const auto& collidersToCheck = currentObjectsToCheck->getColliders();
					if (!collidersToCheck.empty()) {
						if (hasIntersection(colliders, newPosition, collidersToCheck, currentObjectsToCheck->getCurrentPosition())) {
							hasCollision = true;
							break;
						}
					}
