...
namespace Physics {
	...
	public:
		...
		static std::vector<std::shared_ptr<IGameObject>> getDynamicObjectsInArea(const std::shared_ptr<IGameObject>& object);

	private:
		static std::unordered_set<std::shared_ptr<IGameObject>> m_dynamicObjects;
		static std::shared_ptr<Level> m_pCurrentLevel;

		static bool hasCollidersIntersection(const Collider& colliders1, const glm::vec2& last_position1, const glm::vec2& position1,
											 const Collider& colliders2, const glm::vec2& position2);
		static bool hasPositionIntersection(const std::shared_ptr<IGameObject>& pObject1, const glm::vec2& last_position1, const glm::vec2& position1,
